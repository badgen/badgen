    ...
    return `
      <svg width="${width / 10}" height="20" viewBox="0 0 ${width} 200" xmlns="http://www.w3.org/2000/svg"${xlink}>
        <g>
          ...
          <rect x="${sbRectWidth}" width="${stRectWidth}" height="200" fill="#${color}"/>
        </g>
        <g fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">
          ...
          <text x="${sbRectWidth + 55}" y="148" fill="#000" opacity="0.1" textLength="${stTextWidth}">${status}</text>
  ...
  return `
    <svg width="${width / 10}" height="20" viewBox="0 0 ${width} 200" xmlns="http://www.w3.org/2000/svg"${xlink}>
      ...
      <g fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">
        ...
        <text x="${icon ? '215' : '55'}" y="138" textLength="${sbTextWidth}">${subject}</text>
        <text x="${sbRectWidth + 55}" y="148" fill="#000" opacity="0.25" textLength="${stTextWidth}">${status}</text>
        <text x="${sbRectWidth + 45}" y="138" textLength="${stTextWidth}">${status}</text>
      </g>
